...
void send_data_to_bluetooth(const char* data) {
    if (client != -1) {
        ...
        if (bytes_sent == -1) {
            fprintf(stderr, "Failed to send data over Bluetooth\n");
        }
    }
}
...
#if YOLOV5_V62
    if (yolov5.load_param("crowdhumanv5.param"))
        exit(-1);
    if (yolov5.load_model("crowdhumanv5.bin"))
        exit(-1);
#elif YOLOV5_V60
...
static void draw_objects(const cv::Mat& bgr, const std::vector<Object>& objects)
{
    ...
    fprintf(stderr, "Detected objects: %d\n", object_count); 
    std::string object_count_str = std::to_string(object_count);
    send_data_to_bluetooth(object_count_str.c_str());
...
int main(int argc, char** argv)
{
    ...
    capture.open("/home/ubuntu/Downloads/1_1080P_c.mp4");
    // double fps = capture.get(cv::CAP_PROP_FPS);
    double fps = 100;
    auto start_time = std::chrono::steady_clock::now(); 
    bool inference_flag = false; // flag to indicate whether to perform inference
    ...
    while (true)
    {
        ...
        cv::waitKey(1000 / fps); // 等待足够的时间以匹配视频的帧率
    }
