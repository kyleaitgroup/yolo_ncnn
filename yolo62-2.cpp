...
#if YOLOV5_V62
    if (yolov5.load_param("yolo62.param"))
        exit(-1);
    if (yolov5.load_model("yolo62.bin"))
        exit(-1);
#elif YOLOV5_V60
...
int main(int argc, char** argv)
{
    ...
    capture.open(0, cv::CAP_V4L2);  //0-笔记本自带摄像头，1-外接usb双目摄像头
    // capture.set(cv::CAP_PROP_FRAME_WIDTH, 480);  //设置捕获视频的宽度
    // capture.set(cv::CAP_PROP_FRAME_HEIGHT, 480);  //设置捕获视频的高度

    // capture.set(cv::CAP_PROP_FOURCC, cv::VideoWriter::fourcc('M', 'J', 'P', 'G')) ;


    // capture.set(cv::CAP_PROP_FPS, 5); // 設定攝像頭的幀率為 30 幀/秒
